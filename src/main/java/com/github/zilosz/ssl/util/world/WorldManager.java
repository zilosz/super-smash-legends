...
public class WorldManager {
  ...
  public void createWorld(CustomWorldType type, File schematic, Vector pastePoint) {
    ...
    WorldCreator worldCreator = WorldCreator
        .name(type.getWorldName())
        .type(WorldType.FLAT)
        .generateStructures(false)
        .generatorSettings("3;minecraft:air;2");

    com.sk89q.worldedit.Vector worldEditPastePoint =
        new com.sk89q.worldedit.Vector(pastePoint.getX(), pastePoint.getY(), pastePoint.getZ());
    ...
    Section timeConfig =
        SSL.getInstance().getResources().getConfig().getSection("WorldTimeUpdater");
    ...
    worlds.put(type, new WorldInfo(editSession, timeUpdater));
  }

  public void resetWorld(CustomWorldType worldType) {
    Optional.ofNullable(worlds.remove(worldType)).ifPresent(worldInfo -> {
