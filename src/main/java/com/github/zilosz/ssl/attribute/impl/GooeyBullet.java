...
public class GooeyBullet extends RightClickAbility {
  ...
  public void onClick(PlayerInteractEvent event) {
    new GooeyProjectile(config, new AttackInfo(AttackType.GOOEY_BULLET, this)).launch();
    player.getWorld().playSound(player.getLocation(), Sound.SLIME_WALK2, 2, 2);
  }
  ...
  private static class GooeyProjectile extends LivingProjectile<Slime> {
    ...
    public void onBlockHit(BlockHitResult result) {
      entity.getWorld().playSound(entity.getLocation(), Sound.SLIME_WALK, 2, 1);

      ParticleBuilder particle =
          new ParticleBuilder(ParticleEffect.REDSTONE).setColor(new Color(0, 255, 0));
      new ParticleMaker(particle).boom(SSL.getInstance(), entity.getLocation(), 2.5, 0.4, 5);
    }

    @Override
    public void onTargetHit(LivingEntity target) {
      entity.getWorld().playSound(entity.getLocation(), Sound.SLIME_ATTACK, 2, 1);
    }

    @Override
    public void onTick() {
      ParticleBuilder particle =
          new ParticleBuilder(ParticleEffect.REDSTONE).setColor(new Color(124, 252, 0));
