...
public class FrigidSpear extends RightClickAbility {
  ...
  public void onClick(PlayerInteractEvent event) {
    ...
    new SpearProjectile(config.getSection("Projectile"), attackInfo).launch();
    player.getWorld().playSound(player.getLocation(), Sound.GLASS, 1, 2);
  }
  ...
  private static class SpearProjectile extends ItemProjectile {
    ...
    @Override
    public void onTick() {
      new ParticleMaker(new ParticleBuilder(ParticleEffect.SNOW_SHOVEL)).show(entity.getLocation());
    }
  }
}
