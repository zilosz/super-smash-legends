...
public class ArenaVoter extends CustomInventory<Arena> implements HasRandomOption, AutoUpdatesSoft {
  private static final List<String> LORE =
      Arrays.asList("&3&lAuthors: &7{AUTHORS}&7", "&3&lVotes: &f{VOTES}", "");
  ...
  public ItemStack getItemStack(Player player, Arena arena) {
    Replacers replacers =
        new Replacers().add("AUTHORS", arena.getAuthors()).add("VOTES", arena.getTotalVotes());
  ...
  public void onItemClick(
      InventoryContents contents, Player player, Arena arena, InventoryClickEvent event
  ) {
